...
# Prometheus stuff!
graphs = {}
graphs["r_counter"] = Counter(
    "python_request_r_posts", "The total number" + " of register posts."
)
graphs["l_counter"] = Counter(
    "python_request_l_posts", "The total number" + " of login posts."
)
graphs["b_counter"] = Counter(
    "python_request_b_posts", "The total number" + " of card buy posts."
)
graphs["g_counter"] = Counter(
    "python_request_g_posts", "The total number" + " of card gift posts."
)
graphs["u_counter"] = Counter(
    "python_request_u_posts", "The total number" + " of card use posts."
)
graphs["database_error_return_404"] = Counter(
    "database_error_return_404", "The total number" + " of Datat base error 404 returns"
)
...
def gift_card_view(request, prod_num=0):
    ...
    if request.method == "GET":
        ...
        prod = Product.objects.get(product_id=prod_num)
        extras.write_card_data(
            card_file_path,
            prod,
            request.POST.get("amount", prod.recommended_price, user),
        )
        card_file = open(card_file_path, "rb")
        card = Card(
            data=card_file.read(),
